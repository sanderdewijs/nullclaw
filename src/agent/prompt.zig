...
pub fn buildSystemPrompt(
    ...
    // Skills section
    try appendSkillsSection(allocator, w, ctx.workspace_dir, ctx.skill_routing_message, ctx.skill_routing_max_active, ctx.matched_skill_out, ctx.skill_affinity_history, ctx.observer);
...
pub fn buildSkillsSection(allocator: std.mem.Allocator, workspace_dir: []const u8, observer: ?observability.Observer) ![]const u8 {
    ...
    errdefer buf.deinit(allocator);
    const w = buf.writer(allocator);
    try appendSkillsSection(allocator, w, workspace_dir, null, 3, null, null, observer);
    return try buf.toOwnedSlice(allocator);
}
...
test "appendSkillsSection with no skills produces nothing" {
    ...
    defer buf.deinit(allocator);
    const w = buf.writer(allocator);
    try appendSkillsSection(allocator, w, "/tmp/nullclaw-prompt-test-no-skills", null, 0, null, null, null);
...
test "appendSkillsSection renders summary XML for always=false skill" {
    ...
    defer buf.deinit(allocator);
    const w = buf.writer(allocator);
    try appendSkillsSection(allocator, w, base, null, 0, null, null, null);
